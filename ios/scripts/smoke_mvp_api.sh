...
# Smoke-test credentials must come from the environment; no defaults.
: "${PASSWORD:?Set PASSWORD env var for the smoke test}"
...
config="$(curl -fsS --max-time 15 \
  -H "Authorization: Bearer $token" \
  "$BASE_URL/v1/devices/$device_id/config?token=$config_token")"
if ! printf '%s' "$config" | grep -q "^vpn://"; then
  echo "VPN link response is missing vpn:// prefix" >&2
  exit 1
fi
