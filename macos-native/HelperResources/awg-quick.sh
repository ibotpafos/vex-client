...
parse_options() {
	...
	while read -r line || [[ -n $line ]]; do
		...
		if [[ $interface_section -eq 1 ]]; then
			...
			PreUp|PreDown|PostUp|PostDown) die "Lifecycle hooks are prohibited in VEX-managed configs" ;;
			SaveConfig) [[ $value == false ]] || die "SaveConfig is prohibited in VEX-managed configs"; continue ;;
			esac
		fi
		WG_CONFIG+="$line"$'\n'
	done < "$CONFIG_FILE"
...
SERVICE_NAMES=( )
SERVICE_DNS_VALUES=( )
SERVICE_DNS_SEARCH_VALUES=( )

service_index() {
	local service="$1" i
	for ((i = 0; i < ${#SERVICE_NAMES[@]}; ++i)); do
		[[ ${SERVICE_NAMES[$i]} == "$service" ]] && echo "$i" && return 0
	done
	return 1
}

remember_service_dns() {
	local service="$1" dns="$2" search="$3" index
	if index="$(service_index "$service")"; then
		SERVICE_DNS_VALUES[$index]="$dns"
		SERVICE_DNS_SEARCH_VALUES[$index]="$search"
	else
		SERVICE_NAMES+=( "$service" )
		SERVICE_DNS_VALUES+=( "$dns" )
		SERVICE_DNS_SEARCH_VALUES+=( "$search" )
	fi
}

forget_missing_service_dns() {
	local found="$1" names=( ) dns_values=( ) search_values=( ) i
	for ((i = 0; i < ${#SERVICE_NAMES[@]}; ++i)); do
		[[ $found == *$'\n'"${SERVICE_NAMES[$i]}"$'\n'* ]] || continue
		names+=( "${SERVICE_NAMES[$i]}" )
		dns_values+=( "${SERVICE_DNS_VALUES[$i]}" )
		search_values+=( "${SERVICE_DNS_SEARCH_VALUES[$i]}" )
	done
	SERVICE_NAMES=( "${names[@]}" )
	SERVICE_DNS_VALUES=( "${dns_values[@]}" )
	SERVICE_DNS_SEARCH_VALUES=( "${search_values[@]}" )
}

collect_new_service_dns() {
	local service dns_response search_response found_services=$'\n'
	{ read -r _ && while read -r service; do
		[[ $service == "*"* ]] && service="${service:1}"
		found_services+="$service"$'\n'
		service_index "$service" >/dev/null && continue
		dns_response="$(cmd networksetup -getdnsservers "$service")"
		[[ $dns_response == *" "* ]] && dns_response="Empty"
		search_response="$(cmd networksetup -getsearchdomains "$service")"
		[[ $search_response == *" "* ]] && search_response="Empty"
		remember_service_dns "$service" "$dns_response" "$search_response"
	done; } < <(networksetup -listallnetworkservices)

	forget_missing_service_dns "$found_services"
}
...
del_dns() {
	...
	for ((i = 0; i < ${#SERVICE_NAMES[@]}; ++i)); do
		...
		while read -r response; do
			...
			cmd networksetup -setdnsservers "$service" ${SERVICE_DNS_VALUES[$i]} || true
			cmd networksetup -setsearchdomains "$service" ${SERVICE_DNS_SEARCH_VALUES[$i]} || true
		)
	done
}
...
cmd_up() {
	local i
	if get_real_interface; then
		echo "[+] \`$INTERFACE' already exists as \`$REAL_INTERFACE'" >&2
		exit 0
	fi
