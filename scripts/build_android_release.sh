...
if [[ ${#gradle_extra_args[@]} -gt 0 ]]; then
  gradle_command+=("${gradle_extra_args[@]}")
fi
gradle_command+=("$@")
"${gradle_command[@]}"
...
if [[ "${variant}" == "local" ]]; then
  expected_package="${VEX_ANDROID_APPLICATION_ID:-com.vexguard.client}${VEX_DEBUG_APPLICATION_ID_SUFFIX:-.dev}"
