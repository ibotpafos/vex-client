...
grep -q "^package: name='com.vexguard.app' versionCode='1005866' versionName='1.0.58'" <<< "$badging"
...
PY
"$tools" sign --ks "$KEYSTORE_DEST" --ks-key-alias "$VEX_UPLOAD_KEY_ALIAS" --ks-pass env:VEX_UPLOAD_STORE_PASSWORD --key-pass env:VEX_UPLOAD_KEY_PASSWORD --out signed-candidate/VEX-Android-1.0.58.apk "$work/input.apk"
"$tools" verify --verbose --print-certs signed-candidate/VEX-Android-1.0.58.apk > signed-candidate/signature.txt
...
sha256sum signed-candidate/VEX-Android-1.0.58.apk > signed-candidate/checksum.txt
