...
assert.equal(app.android.package, 'com.vexguard.vpn');
assert.equal(app.android.versionCode, 1005658);
assert.equal(app.version, '1.0.56');
assert.match(gradle, /applicationId "com\.vexguard\.vpn"/);
assert.match(gradle, /versionCode 1005658/);
assert.match(config, /env\('VEX_ANDROID_APPLICATION_ID', 'com\.vexguard\.vpn'\)/);
